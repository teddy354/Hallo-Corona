...
func UploadFile(next http.HandlerFunc) http.HandlerFunc {
  ...
  fmt.Println(filetype)
	if filetype != "image/jpeg" && filetype != "image/png" && filetype != "image/jpg" && filetype != "image/jfif"  {
		w.WriteHeader(http.StatusBadRequest)
		response := dto.ErrorResult{Code: http.StatusBadRequest, Message: "The provided file format is not allowed. Please upload a correct image"}
		json.NewEncoder(w).Encode(response)
		return
	}
